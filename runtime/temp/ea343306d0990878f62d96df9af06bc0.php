<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:71:"/www/wwwroot/peigongtang/public/../application/admin/view/Role/add.html";i:1615520764;s:64:"/www/wwwroot/peigongtang/application/admin/view/Public/base.html";i:1610075700;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
            ...
            <form class="layui-form" action="">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="name" maxlength="12" required value="<?php echo !empty($detail['name'])?$detail['name']:''; ?>" lay-verify="required" placeholder="请输入权限组名称" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                                ...
                                <div class="layui-form-item">
                                    <div style="margin-left: 50px;">
                                        <?php if(is_array($vo['_child']) || $vo['_child'] instanceof \think\Collection || $vo['_child'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?>
                                            <input lay-skin="primary" type="checkbox" lay-filter="admin-check" data-id="<?php echo $child['id']; ?>" fid="<?php echo $vo['id']; ?>" data-fid="<?php echo $vo['id']; ?>" name="rule[<?php echo $child['id']; ?>]" value="<?php echo $child['url']; ?>" title="<?php echo $child['name']; ?>" <?php if(isset($hasRule) && in_array($child['url'],$hasRule)): ?>checked<?php endif; ?>>
                                            <?php if(isset($child['_child'])): ?>
                                                <div style="margin-left: 50px;">
                                                    <?php if(is_array($child['_child']) || $child['_child'] instanceof \think\Collection || $child['_child'] instanceof \think\Paginator): $i = 0; $__LIST__ = $child['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$_child): $mod = ($i % 2 );++$i;?>
                                                        <input lay-skin="primary" type="checkbox"  lay-filter="admin-check" data-ffid="<?php echo $vo['id']; ?>" pid="<?php echo $vo['id']; ?>" data-id="<?php echo $_child['id']; ?>" fid="<?php echo $child['id']; ?>" data-fid="<?php echo $child['id']; ?>" name="rule[<?php echo $_child['id']; ?>]" value="<?php echo $_child['url']; ?>" title="<?php echo $_child['name']; ?>" <?php if(isset($hasRule) && in_array($_child['url'],$hasRule)): ?>checked<?php endif; ?>>
