<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"E:\phpEnv\www\qisedu\public/../application/admin\view\Teacher\add.html";i:1621777061;s:61:"E:\phpEnv\www\qisedu\application\admin\view\Public\base2.html";i:1620814255;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
            ...
            <form class="layui-form" action="">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline" style="line-height: 36px;">
                        <input type="text" name="name" maxlength="20" required
                               value="<?php echo !empty($detail['name'])?$detail['name']:''; ?>" lay-verify="required" placeholder="请输入课程名称"
                               class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline" style="width: 150px">
                        <span class="layui-btn" name="file" id="files">上传图片</span>
                        <input type="hidden" id="pics" name="hp_pic"
                               value="<?php if(!(empty($detail) || (($detail instanceof \think\Collection || $detail instanceof \think\Paginator ) && $detail->isEmpty()))): ?><?php echo $detail['qr_code']; endif; ?>">
                        <div id="shows"><?php if(!(empty($detail) || (($detail instanceof \think\Collection || $detail instanceof \think\Paginator ) && $detail->isEmpty()))): ?><img src="<?php echo !empty($detail['hp_pic'])?$detail['hp_pic'] : ''; ?>"
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline" style="width: 150px">
                        ...
                        <input type="hidden" id="pic" name="qr_code"
                               value="<?php if(!(empty($detail) || (($detail instanceof \think\Collection || $detail instanceof \think\Paginator ) && $detail->isEmpty()))): ?><?php echo $detail['qr_code']; endif; ?>">
                        <div id="show"><?php if(!(empty($detail) || (($detail instanceof \think\Collection || $detail instanceof \think\Paginator ) && $detail->isEmpty()))): ?><img src="<?php echo !empty($detail['qr_code'])?$detail['qr_code'] : ''; ?>"
                                                                    height="100"><?php endif; ?>
                        </div>
                    </div>
                    <span style="color:red">
                ...
                <div class="layui-input-block">
                    ...
                    <div class="layui-form-item">
                        <input lay-skin="primary" type="checkbox" data-id="<?php echo $vo['id']; ?>" lay-filter="admin-check" name="rule[<?php echo $vo['id']; ?>]" value="<?php echo $vo['title']; ?>" title="<?php echo $vo['title']; ?>" lay-text="ON|OFF">
                    </div>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label"><span style="color:red">*</span> 选择类型</label>
                    <div class="layui-input-inline" style="width: 500px; ">
                        <select name="type">
                            ...
                            'type']==1): ?> selected <?php endif; endif; ?> value="1">老师</option>
                            <option <?php if(!(empty($detail) || (($detail instanceof \think\Collection || $detail instanceof \think\Paginator ) && $detail->isEmpty()))): if($detail[
                            'type']==2): ?> selected <?php endif; endif; ?> value="2">助教</option>
                        </select>
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">老师或助教简介</label>
                    <div class="layui-input-inline" style="width: 600px;">
                        <textarea class="layui-textarea" name="brief" maxlength="255"
                                  placeholder="老师或助教简介"
...
<block name="myScript">
    <script>
        ...
        // var imageObj = $("#pic");
        var image_path = '<?php echo !empty($detail["qr_code"])?$detail["qr_code"] : ""; ?>';
        var image_paths = '<?php echo !empty($detail["hp_pic"])?$detail["hp_pic"] : ""; ?>';
        if (image_path != '') {
            $("#show").html("<img src='<?php echo !empty($detail["qr_code"])?$detail["qr_code"] : ""; ?>' style='height:100px; margin-top:5px;' />");
        }
        if(image_paths!=''){
            $("#show").html("<img src='<?php echo !empty($detail["hp_pic"])?$detail["hp_pic"] : ""; ?>' style='height:100px; margin-top:5px;' />");
        }
        $("#pic").val(image_path);
        ...
        layui.use('upload', function () {
            ...
            var uploadInst = upload.render({
                ...
                , done: function (res) {
                    ...
                    console.log(res); //上传成功返回值，必须为json格式
                    if (res.type == 1) {
                        $("#shows").html("<img src='" + res.image_path + "' style='height:100px; margin-top:5px;' />");
                        $("#pics").val(res.image_path);
                    } else {
                        tips(res.msg);
                    }
                }
